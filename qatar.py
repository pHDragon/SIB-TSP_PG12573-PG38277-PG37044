...
class Qatar:
    ...
    def iterDA(self, index, how_many = 4):
        ...
        for i in range(how_many):
            ...
            if self.fitness(mutation) <= self.fitness_all[index][worst_fit[i][0]] and not checkIfDuplicates(mutation): # erro comum de duplicar cidades;
    ...
    def permute_islands(self):
        ...
        for i in range(self.islands):
            ...
            if self.fitness(mutation) <= self.fitness_all[next_i][worst_fit_next[0][0]] and not checkIfDuplicates(mutation): # erro comum de duplicar cidades;
    ...
    def increase_diversity(self,i):
        # A população de cada ilha é substituída pela população original, exceto na ilha com melhor fitness
